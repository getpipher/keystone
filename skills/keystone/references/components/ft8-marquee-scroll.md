### Ft8 · Marquee scroll
A horizontal infinite-scroll line of repeating tagline + dot separator: `STUDIO · 2026 · STUDIO · 2026 ·`. Sport-genre sites, fashion lookbooks, brand-forward agencies.
*Use when:* the brand voice is loud, kinetic, sport-or-manifesto.
*Don't confuse with:* Ft4 Dense colophon (which is static text).
**Gates:** G18 (auto-rotating content without pause — marquee needs `:hover` and `:focus-within` pause), G27 (reduced-motion fallback — `animation: none` when `prefers-reduced-motion: reduce`), G14 (animate `transform`, not `width`/`margin`).
