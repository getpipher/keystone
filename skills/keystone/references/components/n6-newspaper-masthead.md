...
Full-width header, large centred wordmark on the top row, thin issue/date line in serif small caps, optional inline link row beneath, double-rule below. Editorial, broadsheet — NYT, FT, Vogue.
*Use when:* the page is editorial, magazine-shaped, or framed as an issue / edition.
*Don't confuse with:* N1 Wordmark + 2 links (asymmetric and small).
**Gates:** G38a (masthead wordmark uses `<h1>` — no italic), G48 (token discipline — `--font-display` for the wordmark), G54 (inline link row beneath is a nav, not a tag-left/heading-right pair).
